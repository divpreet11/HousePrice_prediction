...
# some constants to use for model.yaml file just like we use a seperate constant folder for config.yaml file
...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) :#-> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            model_accuracy = (2 * (train_acc * test_acc)) / (train_acc + test_acc) # this will combine train accuracy and test accuract to give the model accuracy
            diff_test_train_acc = abs(test_acc - train_acc) # give the diffrence between train and test accuracy
...
class ModelFactory:
    def __init__(self, model_config_path: str = None,):
        try:
            ...
            # # As this config has nested dictionary we will have to use this syntax tom reach to thed module
            
            self.grid_search_class_name: str = self.config[GRID_SEARCH_KEY][CLASS_KEY]
             # As this config has nested dictionary we will have to use this syntax tom reach to thed class
             
            self.grid_search_property_data: dict = dict(self.config[GRID_SEARCH_KEY][PARAM_KEY])
            # this will  give the parameter of the grid search 

            self.models_initialization_config: dict = dict(self.config[MODEL_SELECTION_KEY])
            # this willtake us to the model key of model selection for better understanding see the model.yaml file

            self.initialized_model_list = None # this will save the initialized model .. basically the object of model
            self.grid_searched_best_model_list = None # this will save the best grid searched parameter for  a perticular model
    ...
    def read_params(config_path: str):# -> dict
        """ this mwthod will load the model.yaml file and return the parameter in dict format"""
        try:
    ...
    def class_for_name(module_name:str, class_name:str):
        try:
            ...
            module = importlib.import_module(module_name) # This will import the modeule name
            # get the class, will raise AttributeError if class cannot be found
            logging.info(f"Executing command: from {module} import {class_name}")
            class_ref = getattr(module, class_name) # This will do like this module.class_name
            return class_ref
        except Exception as e:
            raise Housing_exception(e, sys) from e
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        try:
            ...
            # This will return sklearn.model_selection._search.GridSearchCV----> go and check 

            grid_search_cv = grid_search_cv_ref(estimator=initialized_model.model,
                                                param_grid=initialized_model.param_grid_search)
            # providing the parameters for the grid search cv
            ...
            grid_search_cv.fit(input_feature, output_feature) # we obtained the grid_search_cv ref now we are fitting the data
    ...
    def get_initialized_model_list(self) -> List[InitializedModelDetail]:
        ...
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys(): # it will run for every model in the model.yaml file

                model_initialization_config = self.models_initialization_config[model_serial_number] # getting the details of every model
                model_obj_ref = ModelFactory.class_for_name(module_name=model_initialization_config[MODULE_KEY],
                                                            class_name=model_initialization_config[CLASS_KEY]
                                                            )
                # This above code will show the output like=[modulename.classname]
                model = model_obj_ref() # as we got the ref of the class , now we will create the object of it
